...
# Description: Automated end-to-end pipeline to compile Prism cPanel Webmail Backup Tool Tool,
...
param (
    ...
    [switch]$AppImageOnly      # Pass -AppImageOnly to stop after creating Prism-cPanel-Webmail-Backup-Tool.exe
)
...
$AppFolder = Join-Path $AppImageDir "Prism-cPanel-Webmail-Backup-Tool"
...
Write-Host "  Prism cPanel Webmail Backup Tool Tool - Full EXE & Installer Pipeline" -ForegroundColor Cyan
...
$JpackageArgs = @(
    "--type", "app-image",
    "--name", "Prism-cPanel-Webmail-Backup-Tool",
...
$ExePath = Join-Path $AppFolder "Prism-cPanel-Webmail-Backup-Tool.exe"
if (-not (Test-Path $ExePath)) {
    Write-Error "Prism-cPanel-Webmail-Backup-Tool.exe was not created at expected location: $ExePath"
}
...
& $AiPath /edit $AipFile /SetProperty ProductName="Prism cPanel Webmail Backup Tool Tool"
...
& $AiPath /edit $AipFile /NewShortcut -name "Prism cPanel Webmail Backup Tool Tool" -dir DesktopFolder -target "[APPDIR]Prism-cPanel-Webmail-Backup-Tool\Prism-cPanel-Webmail-Backup-Tool.exe" -icon "$LogoIcon"
& $AiPath /edit $AipFile /NewShortcut -name "Prism cPanel Webmail Backup Tool Tool" -dir SHORTCUTDIR -target "[APPDIR]Prism-cPanel-Webmail-Backup-Tool\Prism-cPanel-Webmail-Backup-Tool.exe" -icon "$LogoIcon"
...
& $AiPath /edit $AipFile /SetPackageName "Prism-cPanel-Webmail-Backup-Tool-Setup.exe" -buildname DefaultBuild
...
# 10. Sync to Drive
$driveJarDir = "I:\My Drive\Akas\Jars\cPanel Webmail Backup Tool"
if (-not (Test-Path $driveJarDir)) { New-Item -ItemType Directory -Path $driveJarDir -Force | Out-Null }
Copy-Item $ProtectedJar.FullName (Join-Path $driveJarDir "Prism-cPanel-Webmail-Backup-Tool.jar") -Force
...
Copy-Item $AipFile (Join-Path $destAdvance "Prism-cPanel-Webmail-Backup-Tool.aip") -Force
...
Write-Host "     -> Prism-cPanel-Webmail-Backup-Tool-Setup.exe" -ForegroundColor White
Write-Host "  3. Standalone Protected JAR: $driveJarDir" -ForegroundColor White
Write-Host "     -> Prism-cPanel-Webmail-Backup-Tool.jar" -ForegroundColor White
Write-Host "========================================================" -ForegroundColor Green
